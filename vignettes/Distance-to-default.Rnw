...
\end{align}%
%
where $C$ is a European call option. $T$ is the time to maturity, $D_t$ is the debt to due at time $T + t$, and $r$ is the risk free rate. Common choices tend to be $T = 1$ year and $D_t$ as the short term debt plus half of the long term debt. $d_1$ in equation~\eqref{eq:dd} is the so-called distance to default. Equation~\eqref{eq:stock_relation} can be computed with the \verb|BS_call| function. Further, the \verb|get_underlying| can be used to invert the equation~\eqref{eq:stock_relation}
...
The parameters can be estimeted with the \verb|BS_fit| function. The iterative method is used in the following call
...
The maximum likelihood estimator is obtained by maximizing the observed log likelihood%
%
\begin{equation}\label{eq:log_like}\begin{split}
L(\mu,\sigma,\vec{S}) & \propto
  - n \log \Lpar{\sigma^2dt}
  - \sum_{k = 1}^n
    \frac{\Lpar{\log \frac{C^{-1}(S_k, \sigma)}{C^{-1}(S_{k - 1}, \sigma)} - \Lpar{\mu - \sigma^2/2}dt}^2}{\sigma^2dt} \\
%
...
%
where $C^{-1}$ is the inverse of the call option price in equation~\eqref{eq:stock_relation} and implicitly depend on $D_t$, $T$, and $r$. Notice that we need to use $dt$ in \eqref{eq:log_like} and the time to maturity, $T$, in $C$ and $C^{-1}$. The last term in equation~\eqref{eq:log_like} follows from the change of variable%
%
\begin{equation}
\begin{split}
  X &= h^{-1}(Y) \\
...
The iterative method is quite a lot faster though

...
\end{align}%
%
where $C$ is a European call option price, $T$ is the time to maturity, $D_t$ is the debt to due at time $T + t$, and $r$ is the risk free rate. Common choices tend to be $T = 1$ year and $D_t$ as the short term debt plus half of the long term debt.
The distance-to-default is%
%
$$
DD = \frac{\log(V_t) - \log(D_t) + (\mu - \sigma^2 / 2)T}{\sigma \sqrt T}
$$%
%
It is a very good predictor of default risk despite it's simplicity \citep[see e.g.,][]{Bharath08}.

...
%
  ...
  $$\hat{\sigma}^{(i)} = \frac{1}{n}\sum_{k = 1}^n \frac{x_k^2}{dt_k} -
    \left(\frac{1}{n}\sum_{k =1}^n\frac{x_k}{\sqrt{dt_k}} \right )^2
    ,\qquad \hat{\mu}^{(i)} = \frac{1}{n}\sum_{k=1}^n \frac{x_k}{dt_k}
    + \frac{\hat{\sigma}^{(i)2}}{2}$$
    where we have extended to unequal gaps, $dt_k$.
%

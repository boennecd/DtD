...
\end{align}%
%
where $C$ is a European call option price, $T$ is the time to maturity, $D_t$ is the debt to due at time $T + t$, and $r$ is the risk free rate. Common choices tend to be $T = 1$ year and $D_t$ as the short term debt plus half of the long term debt. $d_1$ in equation~\eqref{eq:dd} is the so-called distance-to-default which is the name of the package. It is a very good predictor of default risk despite it's simplicity \citep[see e.g.,][]{Bharath08}.
